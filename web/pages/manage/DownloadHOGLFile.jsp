...
<%
    ...
    if (logFile.exists())
    {
        ...
        try
        {
            ...
            if (DAOFactory.getReportDAO().updateDownloadDetails(filePath, session_userName))
            {
                ...
                System.out.println("Report file - " + fileName + "  username - " + session_userName + ",  download status updatation is successful.");
            }
            else
            {
                System.out.println("Report file - " + fileName + "  username - " + session_userName + ",  download status updatation is unsuccessful.");
            }
            
            DAOFactory.getLogDAO().addLog(new Log(DDM_Constants.log_type_user_download_generated_hogl_report, "| HOGL Report Name - " + logFile.getName() + ", File Type - " + fileExtension + ", Download Status. - Successful , Downloaded By - " + session_userName + " (" + session_userTypeDesc + ") |"));

        }
        catch (IOException e)
        {
            System.out.println(e.getMessage());
            DAOFactory.getLogDAO().addLog(new Log(DDM_Constants.log_type_user_download_generated_hogl_report, "| HOGL Report Name - " + logFile.getName() + ", File Type - " + fileExtension + ", Download Status. - Unsuccessful (" + e.getMessage() + ") , Downloaded By - " + session_userName + " (" + session_userTypeDesc + ") |"));
    ...
    {
        DAOFactory.getLogDAO().addLog(new Log(DDM_Constants.log_type_user_download_generated_hogl_report, "| SLIPS Outward File Name - " + logFile.getName() + ", File Type - " + fileExtension + ", Download Status. - Unsuccessful (File Not Available) , Downloaded By - " + session_userName + " (" + session_userTypeDesc + ") |"));
        response.sendRedirect(request.getContextPath() + "/error.jsp");
    }
%>
